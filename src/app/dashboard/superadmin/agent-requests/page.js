...
export default function AgentRequestsPage() {
  ...
  const approveAgent = async (userId) => {
    if (!confirm("این کاربر را به عنوان مشاور تأیید می‌کنی؟")) return;
    ...
    if (res.ok) {
      alert("مشاور با موفقیت تأیید شد!");
  ...
  const rejectAgent = async (userId) => {
    if (!confirm("درخواست این کاربر را رد می‌کنی؟")) return;
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center">
        <p className="text-2xl text-gray-600">در حال بارگذاری...</p>
      </div>
    );
  }
        ...
        {approved.length > 0 && (
          <section>
            <h2 className="text-3xl font-black text-green-700 mb-6 text-center">
              مشاوران تأیید شده ({approved.length})
            </h2>
              ...
              {approved.map((user) => (
                ...
                >
                  ...
                  <p className="text-sm text-green-600 mt-2">
                    تأیید شده در:{" "}
                    {new Date(user.agentApprovedAt).toLocaleDateString("fa-IR")}
                  </p>
                </div>
              ))}
            </div>
        ...
        {rejected.length > 0 && (
          <section>
            <h2 className="text-3xl font-black text-red-700 mb-6 text-center">
              درخواست‌های رد شده ({rejected.length})
            </h2>
            <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
              {rejected.map((user) => (
                <div
